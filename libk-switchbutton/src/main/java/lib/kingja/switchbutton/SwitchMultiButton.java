...
public class SwitchMultiButton extends View {
    ...
    private static final int UNSELECTED_COLOR = 0xffc6c6c6;
    private static final int SELECTED_TAB = 0;
    private static final String FONTS_DIR = "fonts/";
    /*other*/
    private Paint mStrokePaint;
    private Paint mFillPaint;
    private Paint mUnFillPaint;
    private int mWidth;
    private int mHeight;
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mUnSelectedColor = typedArray.getColor(R.styleable.SwitchMultiButton_unselectedColor, UNSELECTED_COLOR);
    ...
    private void initPaint() {
        ...
        // unselected paint
        mUnFillPaint = new Paint();
        mUnFillPaint.setColor(mUnSelectedColor);
        mUnFillPaint.setStyle(Paint.Style.FILL_AND_STROKE);
        mUnFillPaint.setAntiAlias(true);
        // selected text paint
        mSelectedTextPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        mSelectedTextPaint.setTextSize(mTextSize);
        mSelectedTextPaint.setColor(0xff000000);
        ...
        mUnselectedTextPaint.setColor(0xff000000);
        ...
        for (int i = 0; i < mTabNum; i++) {
            ...
            if (i == mSelectedTab) {
                ...
                if (i == 0) {
                    ...
                    drawOtherRightPath(canvas, top, right, bottom);
                } else if (i == mTabNum - 1) {
                    drawRightPath(canvas, top, right, bottom);
                    drawOtherLeftPath(canvas, left, top, bottom);
                } else {
    ...
    private void drawOtherLeftPath(Canvas canvas, float left, float top, float bottom) {
        Path leftPath = new Path();
        leftPath.moveTo(left + mStrokeRadius, top);
        leftPath.lineTo(perWidth, top);
        leftPath.lineTo(perWidth, bottom);
        ...
        leftPath.lineTo(left, top + mStrokeRadius);
        leftPath.arcTo(new RectF(left, top, left + 2 * mStrokeRadius, top + 2 * mStrokeRadius), 180, 90);
        canvas.drawPath(leftPath, mUnFillPaint);
    }

    private void drawOtherRightPath(Canvas canvas, float top, float right, float bottom) {
        Path rightPath = new Path();
        rightPath.moveTo(right - mStrokeRadius, top);
        rightPath.lineTo(right - perWidth, top);
        rightPath.lineTo(right - perWidth, bottom);
        ...
        rightPath.lineTo(right, top + mStrokeRadius);
        rightPath.arcTo(new RectF(right - 2 * mStrokeRadius, top, right, top + 2 * mStrokeRadius), 0, -90);
        canvas.drawPath(rightPath, mUnFillPaint);
    }
